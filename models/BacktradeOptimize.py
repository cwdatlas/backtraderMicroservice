...
class BacktradeOptimize(BacktradeData):
    ...
    def optimize_validation(self):
        ...
        if start_sma >= end_sma:
            raise MetricOutOfBoundsError('Starting SMA is larger or equal to ending SMA', 'start_sma')
        if start_ema >= end_ema:
            raise MetricOutOfBoundsError('Starting EMA is larger or equal to ending EMA', 'start_ema')
